r"""
Purpose:
Build 15 regression-ready contract-plan-drug cohort panels directly from plan
information and quarterly formulary panels. The analysis unit is contract,
plan, segment, the selected geography level, and NDC.
...
2. Keep plan units that cover every required quarter of each 2020--2024
   cohort, then reproducibly sample the configured fraction of those units.
   Because every formulary-quarter contains every NDC, a sampled balanced plan
   supplies a balanced row for every NDC that survives the formulary inner join.
3. Read each quarterly formulary panel once, merge balanced plans and features,
   and compare raw tier with the immediately preceding calendar quarter.
...
) -> tuple[AnalysisSpec, int, float, int, int, int]:
...
def balanced_plan_units(
    ...
    """Return plan units present in every required quarter of each cohort."""
    result: dict[int, pd.MultiIndex] = {}
    for cohort_year, quarters in cohort_windows.items():
        frames: list[pd.DataFrame] = []
        for year_q in quarters:
            current = crosswalks[year_q][spec.plan_unit_columns].copy()
...
def first_seen_path(time_shift: int) -> Path:
    """Return the global NDC first-seen file aligned to the panel timing."""
    base = FIRST_SEEN_ROOT / "ndc_first_seen.csv"
    return base if time_shift == 0 else base.with_name(f"ndc_first_seen_{shift_label(time_shift)}.csv")


def load_first_seen_lookup(time_shift: int) -> pd.DataFrame:
    """Load one shifted or unshifted global first-included quarter per NDC."""
...
def add_previous_formulary(
    ...
    result["previous_formulary_id"] = pd.NA
...
def main() -> None:
    """Build all 15 balanced req1/Not plan-drug cohort panels directly."""
    (
        spec,
        time_shift,
        sample_fraction,
